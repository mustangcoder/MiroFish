...
def test_project_delete_cascades_simulations_reports_and_tasks(monkeypatch):
    project = _project(ProjectStatus.GRAPH_COMPLETED, graph_id=None)
    events = []
    simulations = [SimpleNamespace(simulation_id="sim-1")]
    ...
    class Simulations:
        ...
        def delete_simulation(self, simulation_id):
            events.append(("simulation", simulation_id))
            return True

    class Reports:
        @classmethod
        def list_reports(cls, simulation_id=None, limit=50):
    ...
    class Tasks:
        ...
        def delete_project_tasks(self, project_id):
            events.append(("tasks", project_id))
            return 2

    monkeypatch.setattr(graph_api, "SimulationManager", Simulations)
    monkeypatch.setattr(graph_api, "ReportManager", Reports)
    monkeypatch.setattr(graph_api, "TaskManager", Tasks)
    monkeypatch.setattr(
        graph_api.ProjectManager,
        "get_project",
    ...
    monkeypatch.setattr(
        ...
        "delete_project",
        classmethod(
            lambda _cls, project_id: events.append(("project", project_id)) or True
        ),
    )
