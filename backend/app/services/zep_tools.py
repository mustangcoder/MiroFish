...
class ZepToolsService:
    ...
    def search_graph(
        ...
        try:
            search = self.client.search if getattr(self, '_backend', 'cloud') == 'graphiti' else self.client.graph.search
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        try:
            node = self._call_with_retry(
                func=(lambda: self.client.get_node(node_uuid)) if getattr(self, '_backend', 'cloud') == 'graphiti' else (lambda: self.client.graph.node.get(uuid_=node_uuid)),
