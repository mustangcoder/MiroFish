...
class BatchSubmission:
    ...
    def __iter__(self):
        return iter(self.episode_uuids)

    def __len__(self):
        return len(self.episode_uuids)
...
class GraphBuilderService:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if self._backend == 'graphiti':
            self.client = get_adapter_client()
    ...
    def create_graph(
        ...
        *,
        ...
        if getattr(self, '_backend', 'cloud') == 'graphiti':
            self.client.create_graph(graph_id, name, "MiroFish Social Simulation Graph")
            return graph_id
        ...
        if getattr(self, '_backend', 'cloud') == 'graphiti':
            ...
            for i in range(0, total_chunks, batch_size):
                ...
                if progress_callback:
                    progress_callback(
                        f"发送第 {i // batch_size + 1}/{(total_chunks + batch_size - 1) // batch_size} 批数据 ({len(batch_chunks)} 块)...",
    ...
    def _wait_for_batch(
        ...
        if getattr(self, '_backend', 'cloud') == 'graphiti':
            if progress_callback:
                progress_callback(f"处理完成: {submission.item_count}/{submission.item_count}", 1.0)
    ...
    def _get_graph_info(self, graph_id: str) -> GraphInfo:
        ...
        if getattr(self, '_backend', 'cloud') == 'graphiti':
            ...
            entity_types = {label for node in nodes for label in (node.labels or []) if label not in {"Entity", "Node"}}
            return GraphInfo(graph_id, len(nodes), len(edges), list(entity_types))
        # 获取节点（分页）
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        ...
        if getattr(self, '_backend', 'cloud') == 'graphiti':
            ...
            nodes_data = [{"uuid": node.uuid, "name": node.name, "labels": node.labels or [], "summary": node.summary or "", "attributes": node.attributes or {}, "created_at": node.created_at} for node in nodes]
            edges_data = [{"uuid": edge.uuid, "name": edge.name or "", "fact": edge.fact or "", "fact_type": edge.name or "", "source_node_uuid": edge.source_node_uuid, "target_node_uuid": edge.target_node_uuid, "source_node_name": node_map.get(edge.source_node_uuid, ""), "target_node_name": node_map.get(edge.target_node_uuid, ""), "attributes": edge.attributes or {}, "created_at": edge.created_at, "valid_at": edge.valid_at, "invalid_at": None, "expired_at": None, "episodes": edge.episodes or []} for edge in edges]
            return {"graph_id": graph_id, "nodes": nodes_data, "edges": edges_data, "node_count": len(nodes_data), "edge_count": len(edges_data)}
