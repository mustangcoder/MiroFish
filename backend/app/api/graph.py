...
def update_project(project_id: str):
    ...
    try:
        project = ProjectManager.update_project_name(project_id, data.get("name", ""))
...
def _delete_project_impl(project_id: str):
    ...
    if related_running_tasks:
        return jsonify({
            "success": False,
            "error": "项目仍有关联任务运行中，暂时不能删除",
        }), 409
    ...
    with graph_guard:
        ...
        try:
            ...
            simulations = simulation_manager.list_simulations(project_id=project_id)
            for simulation in simulations:
                ...
                simulation_manager.delete_simulation(simulation.simulation_id)
            TaskManager().delete_project_tasks(project_id)
            # The local reference remains protected until it is removed, so a
            # new simulation cannot claim the just-deleted graph in between.
            success = ProjectManager.delete_project(project_id)
        except Exception as error:
            logger.exception("彻底删除项目失败 project_id=%s", project_id)
...
def generate_recent_corpus(project_id: str):
    ...
    if not isinstance(years, int) or years <= 0:
        return jsonify({"success": False, "error": "years必须为正整数"}), 400
    if not isinstance(exclude_full_reports, bool):
        return jsonify({"success": False, "error": "exclude_full_reports必须为布尔值"}), 400
    source = ProjectManager.get_extracted_text(project_id)
    if not source:
        return jsonify({"success": False, "error": "未找到提取的文本内容"}), 404
    try:
        cutoff = subtract_years(date.today(), years)
        result = build_recent_corpus(source, cutoff=cutoff, exclude_full_reports=exclude_full_reports)
        artifacts = write_recent_corpus(Path(ProjectManager._get_project_dir(project_id)), result, cutoff=cutoff)
    except ValueError as error:
        return jsonify({"success": False, "error": str(error)}), 400
    summary = {**artifacts.summary, "cutoff_date": cutoff.isoformat(), "output_file": artifacts.output_path.name, "manifest_file": artifacts.manifest_path.name}
    project.active_corpus = 'recent_3y'
    project.corpus_manifest = summary
    ProjectManager.save_project(project)
    return jsonify({"success": True, "data": {"project_id": project_id, "corpus": "recent_3y", **summary}})
...
def _build_graph_impl():
    ...
    try:
        ...
        try:
            text = (
                ProjectManager.get_extracted_text(project_id)
                if corpus == 'full'
                else ProjectManager.get_corpus_text(project_id, corpus)
            )
        except ValueError as error:
            return jsonify({"success": False, "error": str(error)}), 400
        if not text:
        ...
        def build_task():
            ...
            try:
                ...
                effective_batch_size = Config.GRAPH_BUILD_BATCH_SIZE if Config.ZEP_BACKEND == 'graphiti' else 350
                builder.validate_batch_chunks(chunks, batch_size=effective_batch_size)
                ...
                if resume_existing_batch:
                    ...
                    submission = builder.add_text_batches(
                        graph_id,
                        chunks,
                        batch_size=Config.GRAPH_BUILD_BATCH_SIZE if Config.ZEP_BACKEND == 'graphiti' else 350,
...
@graph_bp.route('/task/<task_id>', methods=['PATCH'])
def update_task_display(task_id: str):
    """更新任务名称和备注，不允许修改执行状态。"""
    data = request.get_json(silent=True) or {}
    try:
        updated = TaskManager().update_display(
            task_id,
            data.get("task_type", ""),
            data.get("note", ""),
        )
...
@graph_bp.route('/task/<task_id>', methods=['DELETE'])
def delete_task(task_id: str):
    ...
    if task.status in {TaskStatus.PENDING, TaskStatus.PROCESSING}:
        return jsonify({"success": False, "error": "运行中的任务不能删除"}), 409
...
def list_tasks():
    ...
    if status:
        try:
            ...
            return jsonify({"success": False, "error": f"不支持的任务状态: {status}"}), 400
