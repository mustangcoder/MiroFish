...
MiroFishPlus 会在安全边界持久化长任务进度。页面刷新或服务异常重启后，启动恢复器会复用原项目、模拟、报告和任务 ID，从最近一个完整检查点继续，而不是重新执行整个流程。

| 节点 | 持久化内容 | 恢复行为 |
|------|------------|----------|
| 本体生成 | 已上传文件、提取文本、模拟需求和额外上下文 | 使用原项目继续生成本体，不要求重新上传文件 |
| 图谱构建 | 原图谱 ID、任务 ID、文档块操作标识和确定性 Episode UUID | Zep Cloud 继续查询已提交批次；本地 Graphiti 跳过已确认的文档 Episode |
| 环境搭建 | 每个实体的人设、生成参数、当前阶段和 workflow lease | 只生成缺失人设，然后继续生成模拟配置与初始事件 |
| OASIS 模拟 | 各平台一致性 SQLite 快照、JSONL 字节偏移、完成轮次和动作数 | 截断不完整日志尾部，从最近完整轮次继续，不重放更早轮次 |
| 图谱写入 | 确定性批次键、平台与轮次来源、尝试次数、状态和 Episode UUID | 已确认批次不会重复发送；本地可重试写入能够安全继续 |
| 报告生成 | 大纲、已完成章节、内容摘要、报告 ID 和任务 ID | 复用有效大纲与章节，只生成缺失或损坏的章节 |

工作流元数据与图谱写入账本统一保存在 `backend/uploads/mirofishplus.db`。OASIS 平台数据库属于模拟运行状态，仍保存在各自的模拟目录中，并在每个完整轮次生成一致性检查点副本。

Zep Cloud 写入如果无法确认结果，会被标记为“状态不明确”且不会自动重放，因为远端写操作不保证幂等；这种失败关闭策略可以避免生成重复事实。本地 Graphiti 使用确定性 Episode 标识，因此能够安全恢复。
